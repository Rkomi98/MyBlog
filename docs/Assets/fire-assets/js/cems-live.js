(() => {
  ...
  const STYLE = `
    ...
    * { box-sizing:border-box; }
    ...
    .badge.is-snapshot { color:#fcd34d; background:rgba(245,158,11,.14); border-color:rgba(245,158,11,.4); }
    .badge.is-error { color:#fca5a5; background:rgba(248,113,113,.14); border-color:rgba(248,113,113,.42); }
    .box.is-light .badge.is-snapshot { color:#b45309; }
    .box.is-light .badge.is-error { color:#b91c1c; }
    ...
    .content { display:grid; grid-template-columns:minmax(0,1.35fr) minmax(18rem,.65fr); align-items:start; }
    .main { padding:1.35rem 1.5rem 1.45rem; } .map { position:relative; background:var(--surface,#1e293b); border-left:1px solid var(--border,rgba(148,163,184,.2)); display:flex; align-items:center; justify-content:center; overflow:hidden; padding:1.1rem; }
    ...
    svg { position:relative; width:100%; height:auto; aspect-ratio:600 / 420; overflow:visible; } .title { font-size:1.35rem; font-weight:800; color:var(--text-primary,#e2e8f0); margin:.2rem 0 .5rem; } .reason { color:var(--text-secondary,#cbd5e1); line-height:1.6; font-size:.84rem; margin:0 0 1.1rem; }
